...
function Statictis(props) {
  ...
  const [checkedNewPro, setCheckedNewPro] = useState(false);
  const [checkedRunPro, setCheckedRunPro] = useState(false);
  ...
  const handleCheckboxChange = (item) => {
    if(checkedNewPro === true ){
      checkedArray.includes(item) ? setCheckedArray([...checkedArray]) : setCheckedArray([...checkedArray, item])
    }
    else{
      const updatedArray = checkedArray.filter((checkedItem) => checkedItem !== item);
      setCheckedArray(updatedArray);
    }
    console.log(`Selected item: ${item}`);
    console.log("List render: ", checkedArray);
  };

  console.log("parents: ",checkedNewPro, checkedRunPro)



  // if(agreement == "false"){
  //   setCheckedArray(checkedArray.slice(0));
  // } 
  ...
  return (
    <div className="my-5 mx-28 select-none">
      ...
      <div className="mb-[100px] flex justify-end">
        <Filter handleCheckboxChange={handleCheckboxChange} checkedRunPro={checkedRunPro} setCheckedRunPro={setCheckedRunPro} checkedNewPro={checkedNewPro} setCheckedNewPro={setCheckedNewPro}/>
      </div>
